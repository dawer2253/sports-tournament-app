# ADR-0008: Filtr `status` jedzie listą rozdzieloną przecinkiem
...
skraca. Porządek w panelu musi więc robić filtr, nie usuwanie.
...
chce pokazać turnieje w toku i szkice razem, czyli `draft` **i** `active` naraz.
...
**Zła wartość to `422`.** Endpoint dostaje odpowiedź `ValidationError`, której
dotąd nie miał — `?status=bogus` jest błędem żądania, nie pustą listą. Cichy
fallback do „bez filtra" pokazałby organizerowi pełną listę przy literówce
w adresie.

**Kształty, w których PHP gubi wartości, też są `422`.** Backend odrzuca
powtórzony klucz (`?status=draft&status=active`) i notację nawiasową
(`?status[]=draft`), sprawdzając surowy query string — w `$_GET` ślad po
powtórzeniu już nie istnieje. Bez tego ta decyzja chroniłaby wyłącznie klienta
TS, a `curl` i ręcznie sklejony link dostawałyby `200` z cicho zawężonym
...
pojedynczy. Rozjazd jest świadomy — tam nie ma znanej potrzeby pytania o dwa
stany naraz — ale znaczy, że „`status` w query" nie ma w tym API jednego
kształtu i czytający kontrakt musi spojrzeć na konkretny endpoint. Ten sam
endpoint nie deklaruje też `422`, choć jego `status` również jest wartością
z enuma; domknięcie należy do ticketu, który dowozi mecze, żeby nie mieszać
dwóch endpointów w jednej zmianie.
...
`explode` do typów, więc rozjazd byłby cichy. Dlatego obowiązek stoi w `##
Konwencje` w samym `openapi.yaml`, czyli tam, gdzie się go czyta przy edycji
kontraktu, a nie tylko w tym ADR-ze.
...
`TournamentListGuardTest`, poza walidatorem kontraktu.

Mock ma tu granicę znaną z
